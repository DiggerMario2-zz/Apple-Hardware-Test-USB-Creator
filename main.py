...
from google_services import download_database, search_for_mac
...
def main():
    ...
    # Connect to Google Drive Sheets service search for the suppiled Model of mac
    key = '1JUe733nSpeL6CrqLzeziBoXf7SenvGhOqU1T9Z-PMtA'

    database = download_database(key)
    download_link = search_for_mac(detected_model, database)
    ...
    usb_selection = int(raw_input("Please enter a number: "))
    ...
    if answer:
        ...
        print (subprocess.check_output(['sudo', 'diskutil', 'unmountDisk'
                                        , valid_disk_string]) + set_color('W'))

        # Re-Partition the disk w/ a GUID table and single primary HFS+ J partition
        print valid_disk_string
